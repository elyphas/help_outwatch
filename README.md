...
a `Handler` is both a `Source` and a `Sink`, where a `Source` is analogous to an `Observable` and a Sink is similar to a `PublishSubject.limit(1)`, but with `.next` hidden?
